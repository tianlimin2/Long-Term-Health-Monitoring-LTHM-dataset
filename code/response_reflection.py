...
1. Load data, load memory nodes 'reflection_nodes.json' and 'reflection_embeddings.json', load question-answering dataset 'questions.json'
2. Recall retrieval mechanism that adjusts weight values ​​based on question categories
3. Respond based on retrieved related nodes, and generate a responses file 'responses_answers.json' and a related nodes file 'related_nodes.json'

"""
"""# Important note: The memory_file and embeddings_file are used to load the memory nodes and embeddings.
If you want to response with reflection, use the'reflection_nodes.json' and'reflection_embeddings.json' files.
If you want to response without reflection, replace the memory_file with the 'nodes.json' file, and the embeddings_file with the 'embeddings.json' file.
"""
...
def retrieve(question, embeddings, memory, n_count=5):
    retrieved = {}
    ...
    # RTE = Retrieval with Equal Weights
    gw = [0.33, 0.33, 0.33]
    # RTR = Retrieval with Relevance-Only, if you want to use only relevance, uncomment the line below
    # gw = [0, 1, 0] 
    # RTD = Retrieval with Dynamic Adjustment, there are lot of  changements, so I put those codes in the end of this file.
    master_out = {}
    detailed_scores = {}
...
# RTD = Retrieval with Dynamic Adjustment,
...
# def retrieve_adjusted(question, embeddings, memory, intent, n_count=5):
...
# if __name__ == "__main__":
#     memory_file = 'your.json'
#     embeddings_file = 'your.json'
#     responses_file_path = 'responses_answers.json'
#     related_nodes_file_path= 'related_nodes.json'
#     memory = load_memory(memory_file)
#     embeddings = load_embeddings(embeddings_file)
